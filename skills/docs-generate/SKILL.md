...
name: docs-generate
description: >
  Generate consolidated repository documentation from existing documentation
  found in the current state, unmerged branches, or history, after explicit
  user confirmation and independent verification against the implementation.
  Use for documentation generation that must reuse validated prior material,
  produce docs/ and AGENTS.md, and report agent cost and routing efficiency.
  Never generate documentation when no relevant prior documentation exists.
metadata:
  author: Luan Andryl
  version: "1.0.0"
---
...
## Gate 0 — Mandatory User Confirmation

Before analyzing files, inspecting branches, querying Git history, creating `.docs-analitics/`, spawning agents, running searches, or changing documentation, tell the user that this workflow may consume significant tokens and cost, especially in repositories with many documents, branches, or claims requiring independent validation.

Ask exactly:

```text
Este processo pode disparar vários agentes e consumir uma quantidade
significativa de tokens para analisar documentos, histórico, branches e
validar as conclusões independentemente.

Deseja continuar com a auditoria completa? (sim/não)
```

STOP and wait for an explicit affirmative answer. Do not begin any audit step before confirmation. Treat `sim`, `yes`, `continue`, `pode rodar`, `manda`, `prosseguir`, and clear equivalents as confirmation.

If the answer is negative or ambiguous, return:

```text
Status: CANCELLED_BY_USER

Nenhuma análise do repositório foi realizada.
Nenhum agente foi criado.
Nenhuma documentação foi alterada.
```

Do not try to persuade the user to continue.
...
After user confirmation, discover whether documentation already exists in:
